...
1. The selected vendor-facing architecture is Retell → n8n → dashboard/Juvonno. The direct n8n routes are active: phone inbound routing `/webhook/retell-inbound-clinic-router`, all seven Receptionist functions `/webhook/juvonno-receptionist`, and agent-level `call_analyzed` `/webhook/juvonno`. Published INBOUND version `2d4c8842-c348-4c73-8a37-306dddae2c67` now fetches at most two active destination-phone rows and requires exactly one before it returns any tenant, clinic, or agent scope; duplicate matches return `ambiguous_destination_number`. The operator confirmed Retell-side configuration, but a controlled live callback must still prove that the attached phone maps uniquely to one active clinic and that the expected `call_inbound` response reaches Retell. Do not use the clinic's dedicated Retell agent ID as tenant authority.
